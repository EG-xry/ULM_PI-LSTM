...
Coord_tot = {};         % NEW: To store localization coordinates in desired order [x, z, t]
...
parfor hhh = 1:ProcessNumBlocks  % Now uses user-defined limit
    ...
    % NEW: Reorder coordinate information to output as [x, z, t]
    % Assuming MatTracking columns are: [t, z, x, ...], rearrange to [x, z, t]
    Coord = [MatTracking(:,3), MatTracking(:,2), MatTracking(:,1)];
    Coord_tot{hhh} = Coord;
...
% The columns are arranged as: [x, z, t]
...
% Option 2 (optional): Export coordinate data as CSV (uncomment if needed)
% writematrix(Coord_all, fullfile(workingdir, [filename 'Coordinates.csv']));

fprintf('Localization coordinate information saved in format [x, z, t].\n');

%% Print the Coordinate Output
fprintf('\nCoordinate Output (x, z, t):\n');
disp(Coord_all);
fprintf('Total number of coordinates: %d\n', size(Coord_all, 1));
