...
t_start=tic;
workingdir = [PALA_data_folder '\PALA_data_InSilicoFlow'];
cd(workingdir)
...
ConfigFile = load('PALA_InSilicoFlow_v3_config.mat','MatOut','MyMedia');
MatOut_target=ConfigFile.MatOut;
...
%% Simulated Noise parameters 模拟噪音用 60,40,30,25,20,15,10分贝
...
dB = 20*log10(abs(PALA_AddNoiseInIQ(temp.IQ,NoiseParam)));
dB = dB-max(dB(:));

figure(1);
imData=imagesc(dB(:,:,1),[-30 0]);
colormap gray;
axis image;
colorbar
for ii=1:100
