...
%{
复数数组：如果 MatIn 中的元素是复数，则 abs(MatIn) 返回每个复数的模（即复数在复平面上的距离），计算公式为 sqrt(real(z)^2 + imag(z)^2)
这里计算出 信号的强度。复数，取模，得到的值
%}
...
%% MATRIX CROPPING 裁剪矩阵，去掉上下左右的边，大概是3个像素 %%
...
% 这段代码的作用是 裁剪 MatIn 矩阵，去除边界区域，避免超分辨率定位误差。
% 主要用于 ULM 微泡定位处理，确保 MatInReduced 只包含可用的微泡区域。
...
% 仅仅保留30-50个微泡，这个写死了，一张图片就是30-50微泡。其他的都删除掉。
...
% 微泡 index_mask_z,index_mask_x,frame，三个值都有。一个frame上，最多50个微泡

% chatgpt帮助写的代码
fprintf('检测到 %d 个微泡\n', length(index_mask_z));
for i = 1:length(index_mask_z)
    fprintf('微泡 %d: Z = %d, X = %d, T = %d\n', i, index_mask_z(i), index_mask_x(i), index_numberOfFrames(i));
end

figure; hold on;
scatter(index_mask_x, index_mask_z, 'r.');
xlabel('X 位置');
ylabel('Z 位置');
title('检测到的微泡');
grid on;
...
% 防止一些 空值，0，只能是数字，或者 NaN，明确表述没有，不参与计算
...
for iscat=1:size(index_mask_z,1)
    ...
    % 不同的算法，得到不同的便宜量 Zc，在上面的算法中计算得到结果
