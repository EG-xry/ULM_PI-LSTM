...
def total_loss(model, x_data, measured_vel, x_colloc, beta):
    """
    Compute the total loss:
      L_total = L_data + β L_phys
    """
    ld = data_loss(model, x_data, measured_vel)
    lp = physics_loss(model, x_colloc)
    return ld + beta * lp
...
def train_pinn(model, optimizer, x_data, measured_vel, domain,
               ...
               scheduler=None, adaptive_loss=True, s_data=None, s_phys=None):
    model.train()
    loss_history = []
    ...
    for ep in range(1, epochs+1):
        ...
        if adaptive_loss:
            loss_total = 0.5 * torch.exp(-2 * s_data) * loss_d + 0.5 * torch.exp(-2 * s_phys) * loss_p + s_data + s_phys
        else:
        ...
        if ep % print_every == 0:
            if adaptive_loss:
                print(f"Epoch {ep:05d}: Total Loss = {loss_total.item():.4e}, Data Loss = {loss_d.item():.4e}, Physics Loss = {loss_p.item():.4e}, s_data = {s_data.item():.4e}, s_phys = {s_phys.item():.4e}")
            else:
                print(f"Epoch {ep:05d}: Total Loss = {loss_total.item():.4e}, Data Loss = {loss_d.item():.4e}, Physics Loss = {loss_p.item():.4e}")
    return loss_history
...
def main(args):
    ...
    # Preprocess data to improve quality (if implemented)
    # X, Z, T_norm, measured_vel_np = preprocess_data(X, Z, T_norm, measured_vel_np)
    # print(f"After preprocessing: {len(X)} data points remain")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--csv_file", type=str, default="/Users/eric/Desktop/ULM_PI-LSTM/PINN/PINN Workspace/mb_ulm_4con.csv", 
    ...
    parser.add_argument("--adaptive_loss", action="store_true", help="Use adaptive loss weighting")
    parser.add_argument("--s_data", type=float, help="Scaling factor for data loss")
    parser.add_argument("--s_phys", type=float, help="Scaling factor for physics loss")
